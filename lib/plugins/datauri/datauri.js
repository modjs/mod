...
exports.summary = '图片 URL 转换成 Datauri 格式';
...
var transform = exports.transform = function (input, basePath, igts, sizeLimit, charset) {
    ...
    input = input.replace(imgReg, function (match, file, type) {
        ...
        if(fs.existsSync(fileName)){
            ...
            // jpe jpg jpeg 的 MIME 类型都为 image/jpeg
            ...
            if( image.length > 32 * 1024 ){
                exports.log(fileName , "图片 Base64 编码后数据大于 32K" );
            }
            ...
            exports.log(fileName, "成功内联图片为datauri格式 " );

            return r;

        }else{
            exports.warn(fileName, "找不到被内联的文件" );
            return match;
        }
