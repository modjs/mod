...
exports.runTargets = function(targets, done){
    ...
    var expendedTargets = [];
    // expand nested target to tasks
    targets.forEach(function(task){
        if(_.isString(task) && !tasksConfig[task] && targetsConfig[task]){
            var tasks = normalizeTargets(targetsConfig[task]);
            expendedTargets = expendedTargets.concat(tasks);
        }else{
            expendedTargets.push(task)
        }
    });

    var targetQueue = [];
    expendedTargets.forEach(function(task){
        var name = '';
        var opts = {};
        if(utils.isPlainObject(task)){
            name = task.name;
            opts = task.options || {};
        }else{
            name = task;
        ...
        if(taskName && !targetName){
            ...
            if(!tasksConfig[taskName]){
                logger.error("Unable to find '" + task + "' task")
            }
    ...
    // ready runner queue
    var runnerQueue = targetQueue.map(function(obj){
...
exports.versionCheck = function(range){
    var ver = utils.getVersion();
    if(!require('semver').satisfies(ver, range)){
        logger.error( 'Error: Version '+ ver + ' not satisfied by ' + range + ', update version by "npm install -g modjs@' + range + '".')
    }
};
...
exports.run = function(opts, done){
    ...
    plugins.install(pluginsConfig, function(){
        ...
        if (cmd && (task = exports.loadTask(cmd, rc))) {
            ...
            process.title = "Running target " + cmd;
            title = process.title + ' "' + target + '"';
            console.log( loggerPrefix + title.underline );
...
exports.help = function(cmd){
    if(cmd)
        console.log(("Error: Unable to find '" + cmd + "' task.\n").red);
    else if(config.getBuildFilePath())
        console.log("Error: Unable to find default task.\n".red);
    else
        console.log("Error: Modfile wasn't found to your project.\n".red);
