...
exports.taskRun = function(task, options, done){
    ...
    if(task.run){
        ...
        throw Error('Unsupport task type!');
    }
    ...
    if(targetConfig && Array.isArray(targetConfig.group)){
        ...
        targetConfig.group.forEach(function(itemOptions, index){
            ...
            runnerQueue.push(function(done){
                console.log( (" - Running task " + [taskName, targetName, index].join(":").green).bold );
...
exports.runTargets = function(targets, done){
    ...
    if(!Array.isArray(targets)){
        throw Error("Targets param is illegal");
    }
    ...
    // ready runner queue
    runnerQueue = targetQueue.map(function(t){
        return function(done){

            console.log( (" - Running task " + t.green).bold ) ;
...
// entry point
exports.run = function(cmd, cwd, done){
    var rc = exports.rc = config.load(cwd);
    ...
    plugins.install(pluginsConfig, function(){
        ...
        if (cmd && (task = exports.loadTask(cmd, rc))) {
            logger.needsEnd = true;
            console.log( ("+ Running task " + cmd.green ).bold.underline );
            ...
            console.log( ("+ Running target " + cmd.green + ' "' + target + '"').bold.underline );
