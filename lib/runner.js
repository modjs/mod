    ...
    if(targetConfig && Array.isArray(targetConfig.group)){
        ...
        targetConfig.group.forEach(function(itemOptions, index){
            ...
            runnerQueue.push(function(done){
                logger.info("Runing task: ".yellow + [taskName, targetName, index].join(":").green);
...
exports.runTargets = function(targets, done){
    ...
    targets.map(function(t){
        ...
        if(taskName && !targetName){
            ...
            var expandedTargets = Object.keys(taskConfig).filter(function(key) {
    ...
    // start run queue
    async.series(runnerQueue, done || function (err, result) {
    ...
    plugins.install(pluginsConfig, function(){
        ...
        if (cmd && (task = exports.loadTask(cmd, rc))) {
            ...
            if(!logger.clean_exit){
                exports.taskRun(task, taskOptions, function done(err, str){
                    if (err) {
                        return logger.error(err);
                    }
