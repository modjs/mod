...
exports.run = function (options, done) {
    ...
    try {
        ...
        var replacer = exports.loadTask('replace');
        ...
        exports.files.forEach(function(inputFile){
            ...
            if(file.isDirFormat(dest)){
                ...
                outputFile =  path.join(dest, inputFile);
            }

            // about: http://www.ueber.net/who/mjl/projects/bomstrip/
            if(options['bomStrip']) {
                exports.log("bom strip", inputFile, ">", outputFile);
                replacer.replace(inputFile, outputFile, bomRegex, bomReplace, charset);
                inputFile = outputFile;

            }
            if(options['tabStrip']) {
                exports.log("tab strip", inputFile, ">", outputFile);
                replacer.replace(inputFile, outputFile, tabRegex, tabReplace, charset);
                inputFile = outputFile;
            }
