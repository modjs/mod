...
var transform = exports.transform = function (input, basePath, igts, sizeLimit, charset) {
    ...
    var imgReg = /\/\*[\S\s]*?\*\/|(?:background|border).*url\(\s*\"?\'?(\S*)\.(png|jpg|jpeg|gif|svg\+xml).*\"?\'?\s*\)/gi;

    if(igts) imgReg = /\/\*[\S\s]*?\*\/|(?:background|border).*url\(\s*\"?\'?(\S*)\.(png|jpg|jpeg|gif|svg\+xml)\"?\'?\s*\)/gi;

    input.replace(imgReg, function (match, file, type) {
