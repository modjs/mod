...
var transform = exports.transform = function (input, basePath, igts, sizeLimit, charset) {
    ...
    var replaceReg = /"(?:\\"|[^"])*"|'(?:\\'|[^'])*'|\/\*[\S\s]*?\*\/|\/(?:\\\/|[^/\r\n])+\/(?=[^\/])|\/\/.*|\.\s*(?:background|border).*|(?:^|[^$])\b(?:background|border).*\s*\(\s*(["'])(.+?)\1\s*\)/gi;

    var imgReg = /background|border.*url\(\s*\"?\'?(\S*)\.(png|jpg|jpeg|gif|svg\+xml).*\"?\'?\s*\).+/gi;

    if(igts) imgReg = /background|border.*url\(\s*\"?\'?(\S*)\.(png|jpg|jpeg|gif|svg\+xml)\"?\'?\s*\).+/gi;

    input.replace(replaceReg, function(m, m1){

        if(m1){
            return m.replace(imgReg, function (match, file, type) {
