...
exports.urlRelocate= function(contents, fromBase, toBase, options){

    var urlPrepend = options.urlPrepend;
    var urlRev = options.urlRev;
    var urlRelocateDest = options.urlRelocateDest;
    ...
    return contents.replace(/\/\*[\S\s]*?\*\/|url\(['"]?([^\)]+)['"]?\)/g, function(match, originUrl) {
        ...
        if (utils.isRelativeURI(originUrl)) {
            ...
            if('datauri' in query || 'embed' in query){
                ...
                // copy to dest path
                if(urlRelocateDest){
                    var destPath = path.join(urlRelocateDest, urlRelativeRootPath);
                    file.copy(urlRelativeRootPath, destPath);
                    exports.log(urlRelativeRootPath, '>', destPath);

                    if( urlRev || 'rev' in query) {
...
exports.optimizecss = function(inputFile, outputFile, options, charset){
    ...
    if (contents.length) {
        ...
        // url relocate
        contents = exports.urlRelocate(minimized, urlFromBase, urlToBase, options);
