...
exports.build = function (source, dest, options, done) {
    ...
    // <html data-no-rev>
    var isGlobalRev = options.rev;
    // <html data-url-prepend="http://cdn1.qq.com/">
    var globalUrlPrepend = options.urlPrepend;
    ...
    // attrs walk helper
    function walkAttrs (attrs, func){
        for ( var i = 0, len = attrs.length; i < len; i++ ) {
            var attr = attrs[i];
            var attrName = attr.name.toLowerCase();
            var attrValue = attr.escaped;
            var ret = func(attrName, attrValue);
            if( ret !== undefined){
                return ret;
            }
        }
    ...
    function getUrlPrepend(attrs){
        return walkAttrs(attrs, function(attrName, attrValue){
    ...
    function getStripDefine(attrs){
        return walkAttrs(attrs, function(attrName, attrValue){
            if(attrName === 'data-strip-define' || attrName === 'data-stripdefine'){
                if(attrValue === 'no' || attrValue === 'false'){
                    return false;
                }else{
    ...
    function getGroupSrc(attrs){
        return walkAttrs(attrs, function(attrName, attrValue){
            if(attrName === 'data-group-src'){
                return attrValue;
            }
        })
    }
    ...
    HTMLParser(sourceString, {
        start: function(tag, attrs, unary) {
            ...
            if(loader = isScriptLoader(tag, attrs)){
                ...
                if( loader.name === 'requirejs' ) {
                    ...
                    var compileOptions = {
                        ...
                        loader: loader.name,
                        combine: true,
                        stripDefine: getStripDefine(attrs),
                        mainFile: path.join(path.dirname(relativeRootMainSrc), path.basename(relativeRootMainSrc)),
                ...
                exports.runTask('compile', compileOptions, function(err, content){
                    ...
                    if(loader.embed){
                        ...
                        content = helpers.content(tempPath, {noMinify: options.noMinify});
                        ...
                        file.write(destPath, helpers.content(destPath, {noMinify: options.noMinify}));
                ...
                if(src in sequencedSrc){
                    ...
                    if(Array.isArray( sequencedSrc[src] )){
                        ...
                        if( isEmbed ){
                            ...
                            var sequencedContentsDest = getGroupSrc(attrs);
                            if(!sequencedContentsDest){
                                ...
                                sequencedContentsDest = path.join(sequencedDirname, sequencedName);
                            }
                    ...
                    if( isEmbed ){
                        var embedContents = '';
                        if(tag === 'script'){
                            embedContents = ['<script>', contents, '</script>'];
                            // TODO Hack on error when output: <script src='foo.js'> <script> alert(1) </script>
                            if(prevTag == tag){
                                embedContents.unshift('</script>')
                            }
                            embedContents = embedContents.join('\n');
                        }else if(tag === 'link'){
            ...
            for ( var i = 0, len = attrs.length; i < len; i++ ) {
                ...
                // striping data config value
                if(['data-url-prepend', 'data-urlprepend', 'data-group', 'data-group-src', 'data-stand-alone',
                    'data-standalone', 'data-no-rev', 'data-rev'].indexOf( attrName.toLowerCase() ) !== -1) continue;
    ...
    function outputHTML(){
        ...
        destHTMLContent = helpers.content(destHTMLPath, {noMinify: options.noMinify});
