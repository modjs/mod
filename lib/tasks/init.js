...
exports.copyTemplate = function(templateDir, destDir, data, src, dest, isSrcFile, isDestFile){
    ...
    if(!isDestFile && file.isDirFormat(srcPattern)) srcPattern = [path.join(srcPattern, "**/.*"), path.join(srcPattern, "**/*")];
    ...
    file.glob(srcPattern).forEach(function(inputFile){
        ...
        if(!isDestFile && file.isDirFormat(dest))
...
exports.runGenerator = function(template, dest, data, done){
    ...
    var templateDir;
    var buildInGenerators = ['json', 'jquery', 'mocha', 'git', 'modfile', 'readme', 'plugin', 'licenses'];
