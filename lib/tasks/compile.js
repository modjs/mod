 ...
 */
exports.stripDefine = function(mainModuleName, varModules, stubModules, name, path, contents){
    // plugin module like tmpl
    ...
    varModules = [].concat( varModules || 'var').join('|'); // default
    if(name == 'tmpl') console.log(name, path, contents)
    if ( new RegExp("\/?("+varModules+")\/").test( name ) ) {
        var namespace = /.*\/([\w-_\$]+)/.exec(name)[1];
        contents = contents
            .replace( /define\([\w\W]*?return/, function(){   // do not use string replacer with unkown namespace like $$
...
exports.compileRequireJS = function (inputFile, outputFile, options, callback) {
    ...
    if(options.stripDefine){
        ...
        config.onBuildWrite = exports.stripDefine.bind(this, options.mainModuleName, options.varModules, options.stubModules);
