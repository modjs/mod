 ...
 */
exports.stripDefine = function(name, path, contents){
    var rdefineEnd = /\}\)[;^}\w]*$/;
    // Convert var modules
    if ( /.\/var\//.test( path ) ) {
        contents = contents
            .replace( /define\([\w\W]*?return/, "var " + (/var\/([\w-_\$]+)/.exec(name)[1]) + " =" )
            .replace( rdefineEnd, "" );

    } else {

        // TODO: Ignore main point's return statement (the only necessary one)
        contents = contents
...
exports.compileRequireJS = function (include, outputFile, options, callback) {
    ...
    if(options.stripDefine){
        ...
        config.onBuildWrite = exports.stripDefine;
