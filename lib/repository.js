...
var couchdb = require('./utils/couchdb'),
...
exports.updateDoc = function (doc, user, time, cfg, tfile, readme, callback) {
    doc.time.modified = format.ISODateString(time);
    doc.time[cfg.version] = format.ISODateString(time);
...
exports.createDoc = function (user, time, cfg, tfile, readme, callback) {
    var doc = {
        ...
        versions: {},
        time: {
            created: format.ISODateString(time)
        },
        activity: []
    };
...
exports.publish = function (path, repository, /*opt*/options, callback) {
    ...
    packages.loadPackageJSON(path, function (err, cfg) {
        if (err) {
            return callback(err);
        }
        async.series({
            ...
            session: function (cb) {
                // session 相对 db_url 地址
                couchdb(repository).session(cb);
            }
        },
...
exports.unpublish = function (repository, name, version, options, callback) {
    ...
    function removeVersion(v, data) {
        ...
        delete data.time[v];
        data.time.modified = format.ISODateString(new Date());
 ...
 *
 * @param {String} name - the name of the package to lookup
 * @param {String|Array} ranges - acceptable version range (or array of ranges)
...
exports.fetch = function (name, version, repos, callback) {
    ...
    cache.get(name, version, function (err, tarfile, cachedir) {
        if (cachedir && tarfile) {
            packages.loadPackageJSON(cachedir, function (err, cfg) {
                if (err) {
                    return callback(err);
                }
        ...
        exports.resolve(name, version, repos, function (err, v, cfg, repo) {
            ...
            cache.get(name, v, function (err, c_tfile, c_cdir) {
                ...
                logger.info('downloading', auth.noAuthURL(url));
...
// @see server/_design/repository.json
exports.search = function (repo, q, /*optional*/limit, callback) {
    ...
    var s_url = url.resolve(
        repo, '_design/repository/_view/search'
    );


    var qs = {
        key: '"'+q+'"', //CouchDB 的 key参数值必须以双引号包裹，否则报400错误  http://127.0.0.1:5984/repository/_design/repository/_view/search?key="jquery"
        limit: limit,
        include_docs: true
    };

    logger.debug('Search at', auth.noAuthURL(s_url));
    logger.debug('Search querystring', JSON.stringify(qs));
    request.get({
        url: s_url,
        json: true,
        qs: qs,
        proxy: utils.getHttpProxy(s_url)
