...
exports.build = function build(inputFile, outputFile, options){
    // reset build data
    build.mainId = null;
    transport.records = [];
    ...
    if(options.mainConfigFile){
        ...
        configData.plugins.forEach(function(pluginName){
            var pluginPath = path.resolve(__dirname, '../../node_modules/seajs/dist/plugin-'+ pluginName);
            if(file.exists(pluginPath)){
                plugins.push('// plugin-' + pluginName);
                plugins.push(file.read(pluginPath));
            }else{
                console.log(pluginName, 'plugin not combine');
            }
        });
        ...
        // TODO: function cant stringify
        delete configData.plugins;
        configContents = ['seajs.config(', JSON.stringify(config), ');'].join('');
    }
...
function transport(inputFile, options){
    // inspired by https://github.com/spmjs/grunt-cmd-transport
    ...
    if (!meta) {
        return console.warn('found non cmd module "' + inputFile + '"');
    }

    if (meta.id) {
        console.warn('found id in "' + inputFile + '"');
        return contents;
    }
    ...
    var depContents = meta.dependencies.map(function(dep) {
        if (dep.charAt(0) === '.') {
            ...
            if (!file.exists(fpath)) {
                console.warn('file ' + fpath + ' not found');
                return '';
            }
    ...
    return [contents, depContents].join('\n');
}
...
function moduleDependencies(id, options) {
    ...
    if (!fpath) {
        console.warn("can't find module " + alias);
        return [];
    }
    if (!file.exists(fpath)) {
        console.warn("can't find " + fpath);
