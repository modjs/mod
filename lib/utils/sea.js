// transport inspired by https://github.com/spmjs/grunt-cmd-transport
...
function build(inputFile, options){
    ...
    if(options.mainConfigFile){
        ...
        configData.plugins.forEach(function(pluginName){
            var pluginPath = path.resolve(__dirname, '../../node_modules/seajs/dist/plugin-'+ pluginName + '.js');
            if(file.exists(pluginPath)){
                plugins.push('// plugin-' + pluginName);
                plugins.push(file.read(pluginPath));
            }else{
                logger.log(pluginName, 'plugin not combine');
            }
        });
...
function moduleDependencies(id, options) {
    ...
    if (!fpath) {
        logger.warn("can't find module " + alias);
        return [];
    }
