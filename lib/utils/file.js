...
function copydir(srcDir, destDir, opts, copiedFiles) {
    ...
    for (i = 0; i < files.length; i++) {
        ...
        }else if(exports.isDir(srcFileName) && opts.recursive){

            destDir = path.join(destDir, path.basename(srcFileName));
            copydir(srcFileName, destDir, opts, copiedFiles)
        }
    }
...
function copyFile(srcFile, destFile, opts) {
    ...
    if (opts.force && exports.exists(destFile)) {
        ...
        if(opts.backup){
            copyFile(destFile, destFile + "." + utils.getFullDate() + ".bak", opts);
        }
        // If force is new, then compare dates and only copy if the src is newer than dest.
        if ( opts.force == "new" && fs.statSync(destFile).mtime.getTime() >= fs.statSync(srcFile).mtime.getTime()) {
            return false;
        }
    }
